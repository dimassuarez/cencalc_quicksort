...
!-----------------------------------------------------------------------------------------
      ...
      integer, dimension(:), allocatable:: free               ! pointer to free dihedrals 
      integer, dimension(:), allocatable:: qmono              ! pointer to quasi-monomodal dihedrals
      integer, dimension(:), allocatable:: rare               ! pointer to rare dihedrals
      integer, dimension(:), allocatable:: corr               ! pointer to most-likely correlated dihedrals 
      integer, dimension(:,:), allocatable  :: corr_group     ! Von Mises PDF for plotting purposes
      integer, dimension(:), allocatable:: itmpaux            ! auxiliary array
      integer, dimension(:), allocatable:: ngroup             ! auxiliary array
      character*512 freecol,rarecol,qmonocol,corrcol,tmpcol
      character*512 corrcol_group(maxgroup)
...
!  Allocating Arrays
      ...
      allocate(free(NumFiles))
      allocate(qmono(NumFiles))
      allocate(rare(NumFiles))
      allocate(corr(NumFiles))
      allocate(itmpaux(NumFiles))
...
!
      if ( RdBigMat ) then 
         write(*,'(A20,1X,A60)') "Reading BIGMATRIX in ",BigMatFile
...
!
!     Writing BigMatrix if requested and the stops
!
      if ( WrBigMat   ) then 
         write(*,'(A20,1X,A60)') " Writing BIGMATRIX in ",BigMatFile
...
!--SELECTING THOSE COLUMNS OF BigMatrix THAT CHANGE CONFORMATIONALLY
      ...
      print*,"  "
!
!     Detecting Free ( and relatively Fast ) torsions. There may be
!     slow torsions that more or less accidentally have nearly maximum entropy, but they
!     can be correlated with other slow torsions
!   
      freq_thres=0.10d0*maxval(freq)
      nfree=0
      do i=1,NewNumCol
         k=active(i)
         if ( FNumMins(k) .ge. 2 )  then 
              nmin=FNumMins(k)
              sfree=-R*dfloat(nmin)*(1.0d0/dfloat(nmin))*log(1.0d0/dfloat(nmin))
              if((abs(s1(k)-sfree).le.s1_limit) .and. (freq(k).ge.freq_thres)) then 
                  nfree=nfree+1
                  free(nfree)=i
              endif
         endif
      enddo
      if ( nfree .gt. 0 )  then 
        write(6,'(''% Free dihedrals nfree='',I4)') nfree
        call COLSPEC(nfree,free,l,freecol)
        do i=1,nfree
           k=active ( free(i) )
           write(6, '(''% '',A9, A50,'' S1='',F8.3,'' FREQ='',F12.8)') &
           Filelist(k), IdDiH(k), s1(k), freq(k)
        enddo
        write(6, '(''% Free dihedrals USECOL='',A )') freecol(1:l)
      endif
!
!    Detecting quasi-monomodal torsions
!
      nqmono=0
      do i=1,NewNumCol
         k=active(i)
         if ( maxval(Pop(1:FNumMins(k),k)) .ge. 99.0d0 ) then 
                  nqmono=nqmono+1
                  qmono(nqmono)=i
         endif
      enddo
      if (  nqmono .gt. 0 ) then 
        write(6,'(''% Quasi-monomodal dihedrals (Dominant Population > 99%) nqmono='',I4)') nqmono
        call COLSPEC(nqmono,qmono,l,qmonocol)
        do i=1,nqmono
           k=active ( qmono(i) )
           write(6, '(''% '',A9, A50,'' S1='',F8.3,'' FREQ='',F12.8)') Filelist(k), IdDiH(k), s1(k), Freq(k)
        enddo
        write(6, '(''% Quasi-monomodal dihedrals  USECOL='',A )') qmonocol(1:l)
      endif
!
!    Detecting rare torsions
!  
      freq_thres=99999999.0d0
      do i=1,NewNumCol
           if (freq( active(i)).lt.freq_thres) freq_thres=freq( active(i))
      enddo
      freq_thres=10.0d0*freq_thres
!
      nrare=0
      do i=1,NewNumCol
         k=active(i)
         if ( (maxval(Pop(1:FNumMins(k),k)) .lt. 99.0d0 ).and.(freq(k).le.freq_thres))  then 
                  nrare=nrare+1
                  rare(nrare)=i
         endif
      enddo
      if ( nrare .gt. 0 )  then 
        write(6,'(''% Rare dihedrals (Multimodal, but freq < '',F12.8,'') nrare='',I4)') &
        freq_thres,nrare
        call COLSPEC(nrare,rare,l,rarecol)
        do i=1,nrare
           k=active ( rare(i) )
           write(6, '(''% '',A9, A50,'' S1='',F8.3,'' FREQ='',F12.8)') Filelist(k), IdDiH(k), s1(k),Freq(k)
        enddo
        write(6, '(''% Rare dihedrals  USECOL='',A )') rarecol(1:l)
      endif
      write(6,'(''% Free/Quasi-Monomodal dihedrals ONLY for marginal entropies'')') 
      write(6,'(''% Rare dihedrals maybe more problematic'')') 
!
!     Marking most-likely correlated dihedrals
!
      do i=1,NewNumCol
        itmpaux(i)=1
      enddo
...
!
      if ( ncorr .gt. 0 ) then 
        write(6,'(''% Most-likely correlated dihedrals  ncorr='',I4)') ncorr
        call COLSPEC(ncorr,corr,l,corrcol)
        do i=1,ncorr
           k=active ( corr(i) )
           write(6, '(''% '',A9, A50,''S1='',f8.3,'' FREQ='',F12.8)') Filelist(k), IdDiH(k),s1(k),Freq(k)
        enddo
        write(6, '(''% Most-likely correlated dihedrals USECOL='',A )') corrcol(1:l)
      endif
!
...
!
      ...
      character legtext*30, legmin*30
...
!
      CALL GRAF(0.D0, dfloat(NumSnap), 0.D0, dfloat(NumSnap)/5.0d0, 0.D0, 4.D0, 0.D0, 1.D0)
      CALL SETRGB(0.7D0, 0.7D0, 0.7D0)
